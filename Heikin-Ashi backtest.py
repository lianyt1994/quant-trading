...
# basically we do a few transformations on four key benchmarks - Open, 收盘, 最高, 最低
# apply some unique rules on ha Open, 收盘, 最高, 最低 to trade
...
def heikin_ashi(data):
    ...
    # heikin ashi close
    df['HA close'] = (df['开盘'] + df['收盘'] + df['最高'] + df['最低']) / 4

    # initialize heikin ashi open
    df['HA open'] = float(0)
    df['HA open'][0] = df['开盘'][0]
    ...
    temp = pd.concat([df['HA open'], df['HA close'], df['最低'], df['最高']], axis=1)
...
def candlestick(df, ax=None, titlename='', highcol='最高', lowcol='最低',
                opencol='开盘', closecol='收盘', xcol='日期',
    ...
    # only show 5 xticks
    plt.xticks(range(0, len(df), len(df) // 5), df[xcol][0::len(df) // 5])
    plt.title(titlename)
...
def plot(df, ticker):
    ...
    candlestick(df, ax1, titlename='', highcol='HA high', lowcol='HA low',
                opencol='HA open', closecol='HA close', xcol='日期',
    ...
    df['收盘'].plot(ax=ax2, label=ticker)
    ...
    ax2.plot(df.loc[df['signals'] == 1].index, df['收盘'][df['signals'] == 1], marker='^', lw=0, c='g', label='long')
    ax2.plot(df.loc[df['signals'] < 0].index, df['收盘'][df['signals'] < 0], marker='v', lw=0, c='r', label='short')
...
def portfolio(data, capital0=10000, positions=100):
    ...
    portfolio['holdings'] = data['cumsum'] * data['收盘'] * positions
    portfolio['cash'] = capital0 - (data['signals'] * data['收盘'] * positions).cumsum()
...
def stats(portfolio, trading_signals, stdate, eddate, capital0=10000):
    ...
    benchmark = ef.stock.get_quote_history('SPY')

    # return of benchmark
    return_of_benchmark = float(benchmark['收盘'].iloc[-1] / benchmark['开盘'].iloc[0] - 1)
...
def main():
    ...
    # downloading data
    df = ef.stock.get_quote_history('TSLA')
